...
class CommentController extends Controller
{
    ...
    public function zhIndex($productId)
    {
        try {
            $product = ZhProduct::find($productId);

            if (!$product) {
                return responseJson(null, 404, 'Product not found');
            }

            $comments = $product->zhComments;
    ...
    {
        try {
            $product = ZhProduct::find($request->zh_product_id);
            ...
            $comment = new ZhComment($validated);
            ...
            $comment->zh_product_id = $product->id;
            $comment->date = now();
            $comment->save();
